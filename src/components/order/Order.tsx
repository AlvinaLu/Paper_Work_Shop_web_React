...
import {
  ...
  Grid,
  makeStyles,
  Paper, Radio, RadioGroup, RadioProps,
  TextField,
...
import {cookies, expiredTime, NewUser} from "../login/SingUp";
...
export const Order = () => {
  ...
  const [order, setOrder] = useState<IOrder>({firstName: "", lastName: "", postalCod: "", address: "", city:"", number: "", info:""});
    ...
    NewOrderRequest(selectedValue, selectedValuePayment, firstName, lastName, postalCod, address, city, number, info )
      .then(result => {
        if (result.status == 200) {
          ...
          console.log("Order is wrong" + result.errorText);
